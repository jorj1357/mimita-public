...
* violation on the main thread. Verifies a nonzero .txt report and a valid
...
int main()
{
    ...
    // Verify the crash dir exists before we crash.
    ...
    // Intentional main-thread access violation.
    volatile int* p = nullptr;
    int x = *p; // ACCESS_VIOLATION
    (void)x;

    // Unreachable if the handler returns CONTINUE_SEARCH (process terminates).
    printf("FAIL: process did not terminate after access violation\n");
    return 2;
}
