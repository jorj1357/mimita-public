...
const routes = {
    ...
    "/api/coordinator/ice/host-poll": async (req, res) => {
        ...
        for (const [rid, conn] of room.connections) {
            if (now - conn.created_at > REQUEST_TIMEOUT_MS) {
                room.connections.delete(rid); // cleanup expired
                continue;
            }
    ...
    "/api/coordinator/ice/client-poll": async (req, res) => {
        ...
        if (conn.status === "answered" && conn.host_peer_sdp) {
    ...
    "/api/coordinator/ice/request-complete": async (req, res) => {
        ...
        if (room) {
            room.connections.delete(body.request_id);
            console.log("[ICE REQUEST COMPLETE] code=" + body.room_code + " req=" + (body.request_id || "").substring(0,12) + "...");
        }
        json(res, 200, { ok: true });
    },
