...
def extract_texture_from_name(name):
    # expects: textureNameCube.xxx
    if "Cube" in name:
        return name.split("Cube", 1)[0]
    return "default"
...
def run_inside_blender():
    ...
    for obj in bpy.context.scene.objects:
        ...
        # cubes v2
        # textures are in the name of blocks in belnder
        # might not be good scalable but hey wtv 
        if "Cube" in name:
            export["blocks"].append({
                "name": name,
                "pos": snap_position(obj.location),
                "size": [
                    snap(obj.dimensions.x, POS_STEP, 1),
                    snap(obj.dimensions.y, POS_STEP, 1),
                    snap(obj.dimensions.z, POS_STEP, 1),
                ],
                "tex": extract_texture_from_name(name)
            })
