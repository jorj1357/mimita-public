...
PlayMenuResult drawPlayMenu(GLFWwindow* win)
{
    ...
    if (uiButton(win, "DUELS",
        layout.getRectDesign("DUELS", {800.0f, 470.0f, 320.0f, 68.0f}),
        {0.9f, 0.28f, 0.12f, 1.0f}).clicked)
    {
        r.goDuels = true;
    }
    uiDrawText("Fast 1v1 and PvE combat.", uiScaleX(825.0f), uiScaleY(550.0f), 0.32f,
               {0.72f, 0.78f, 0.88f, 1.0f});

    // ONLINE / COMMUNITY SERVERS
    if (uiButton(win, "ONLINE / COMMUNITY SERVERS",
        layout.getRectDesign("ONLINE / COMMUNITY SERVERS", {800.0f, 615.0f, 320.0f, 58.0f}),
        {0.22f, 0.5f, 0.78f, 1.0f}).clicked)
    {
        r.goOnline = true;
    }
    uiDrawText("Play with other people.", uiScaleX(825.0f), uiScaleY(683.0f), 0.32f,
               {0.72f, 0.78f, 0.88f, 1.0f});

    // PRACTICE
    if (uiButton(win, "PRACTICE",
        layout.getRectDesign("PRACTICE", {800.0f, 723.0f, 320.0f, 58.0f}),
        {0.25f, 0.65f, 0.45f, 1.0f}).clicked)
    {
        r.goPractice = true;
    }
    uiDrawText("Sandbox and training modes.", uiScaleX(825.0f), uiScaleY(791.0f), 0.32f,
               {0.72f, 0.78f, 0.88f, 1.0f});
