                            ...
                            SnapshotTransform& out)
{
    ...
    // faster than the server, or a delayed snapshot), clamp so the body is
    // pinned to the newest real position instead of extrapolating a guessed
    // "where the player is going" lead. A permanent lead is what made remote
    // bodies appear offset ahead of the player's actual position while moving.
    const double cappedRenderTick = std::min(
        desiredRenderTick, (double)newestTick);
    ...
    const double renderTick = (prevRenderTick > cappedRenderTick)
        ? prevRenderTick : cappedRenderTick;
                         ...
                         bool showDisagreement)
{
    ...
    RagdollModeSystem::instance().removeCorpsesForOwner(entityId);
                              ...
                              int healthAfter, bool killed, bool npc)
{
    ...
    interpolation.pendingPredictedDamage = 0;
    // A confirmed kill means the entity died and its server-respawned body
    // immediately returns to positive authoritative health (NPC respawn is
    // instant, so the client may never see a health-0 snapshot). Keeping the
    // cap at 0 would pin displayHealth at 0 until the attack-request timeout,
    // hiding the respawned body. Clear the overlay entirely so the
    // authoritative snapshot health drives the display directly.
    if (killed)
        interpolation.predictedHealthCap = -1;
    else
