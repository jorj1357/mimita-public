...
// ── Standard coordinator operations (unchanged logic, now with JSON) ──
    ...
    int maxPlayers)
{
    ...
    std::string body = "{"
        "\"host_session_id\":\"" + jsonEscape(hostSessionId) + "\","
        "\"public_ip\":\"" + jsonEscape(publicIp) + "\","
        "\"port\":" + std::to_string(port) + ","
        "\"server_name\":\"" + jsonEscape(serverName) + "\","
        "\"map\":\"" + jsonEscape(mapName) + "\","
        "\"gamemode\":\"" + jsonEscape(gamemode) + "\","
    ...
    if (httpPostJson(url, body, response, 5000)) {
        try {
            auto j = json::parse(response);
            info.code = jsonStr(j, "code");
            info.joinToken = jsonStr(j, "join_token");
        } catch (...) {}
        printf("[COORDINATOR] registered code=%s\n", info.code.c_str());
    }
    return info;
}
...
{
    std::string body = "{\"code\":\"" + jsonEscape(code) + "\",\"players\":" + std::to_string(playerCount) + "}";
    std::string response;
    bool ok = httpPostJson(gCoordinatorUrl + "/api/coordinator/heartbeat", body, response, 3000);
    return ok;
}
...
bool coordinatorLeave(const std::string& code)
{
    ...
    std::string response;
    bool ok = httpPostJson(gCoordinatorUrl + "/api/coordinator/leave", body, response, 3000);
...
CoordinatorLookupResult coordinatorLookup(const std::string& code)
{
    ...
    std::string body = "{\"code\":\"" + jsonEscape(code) + "\"}";
    std::string response;
    if (httpPostJson(gCoordinatorUrl + "/api/coordinator/lookup", body, response, 5000)) {
        result.reachable = true;
        try {
            auto j = json::parse(response);
            std::string s = jsonStr(j, "status");
            result.exists = !s.empty() && s != "null_r";
            if (result.exists) {
                result.serverName = jsonStr(j, "server_name");
                result.map = jsonStr(j, "map");
                result.gamemode = jsonStr(j, "gamemode");
...
CoordinatorJoinResult coordinatorJoin(const std::string& code, const std::string& playerName)
{
    ...
    std::string body = "{\"code\":\"" + jsonEscape(code) + "\",\"player_name\":\"" + jsonEscape(playerName) + "\"}";
    std::string response;
    if (httpPostJson(gCoordinatorUrl + "/api/coordinator/join", body, response, 5000)) {
        try {
            auto j = json::parse(response);
            result.joinToken = jsonStr(j, "join_token");
            result.serverIp = jsonStr(j, "server_ip");
            result.serverPort = (uint16_t)jsonInt(j, "server_port");
            result.serverName = jsonStr(j, "server_name");
...
{
    std::string body = "{\"code\":\"" + jsonEscape(code) + "\",\"join_token\":\"" + jsonEscape(joinToken) + "\"}";
    std::string response;
    if (httpPostJson(gCoordinatorUrl + "/api/coordinator/join-validate", body, response, 5000)) {
        try {
            auto j = json::parse(response);
            return jsonBool(j, "valid");
        } catch (...) {}
    }
    return false;
}
