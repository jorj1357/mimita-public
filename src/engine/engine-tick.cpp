...
// ---- Globals (externed from main.cpp via terminal-state.h / direct decl) ----
...
void engineTick(
    ...
    MimitaNet::MultiplayerContext& mpContext,
    double& simAccumulator
) {
    constexpr double SIM_DT = 1.0 / 60.0;
    Player& player = *simContext.player;
    World& world = *simContext.world;
    NpcSystem& npcSystem = *simContext.npcSystem;        HotReloadSystem::instance().reloadGameDLLIfChanged();
        ...
        if (gameState == GAME_PLAYING || replayExportForceRender)
        {
            ...
            if (DebugConfig::DEBUG_PHYSICS && (!gReplayExportRenderMode || ReplayExportUI::showSpeedDisplay))
            {
                ...
                snprintf(dbg, sizeof(dbg),
                    "gnd:%d stb:%d realC:%d wc:%d pos:(%.1f %.1f %.1f) vel.z:%.1f",
                    (int)player.onGround, (int)player.stableOnGround,
                ...
                uiDrawText(dbg, 24.0f, (float)y, 0.28f, {0.3f, 1.0f, 0.6f, 1.0f}); y += 16;
                snprintf(dbg, sizeof(dbg),
                    "didLand:%d gLost:%.2f airT:%.2f",
                    (int)player.didLand, player.groundLostTimer, player.airborneTimer);
