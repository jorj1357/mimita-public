...
void glbPhaseSweepSlide(Player& p, const World& world, bool& groundedThisFrame, float dt,
    const glm::vec3& totalMove, glm::vec3& remainingMove,
    std::vector<int>& bodyCandidateExtras,
    const std::vector<glm::vec3>& bodyDeltas,
    ...
    CollisionTraceSnapshot& trace)
{
    ...
    for (int iter = 0; iter < 5; ++iter)
    {
        ...
        std::vector<int> candidates = gatherGLBTriangles(world, cap, remainingMove);
...
void glbPhaseBodyWeaponPush(Player& p, const World& world, bool& groundedThisFrame,
    CollisionTraceSnapshot& trace)
{
