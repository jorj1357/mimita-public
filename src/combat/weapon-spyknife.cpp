...
// ── Knife capsule from right arm world transform ──────────────
...
// ── Swept blade vs single body-part sphere ────────────────────
                              ...
                              const World& world, float dt)
{
    ...
    if (state.active) {
        for (uint32_t t = 0; t < ticksThisFrame; t++) {
            ...
            // ── Swept blade collision against NPCs ──
    ...
    // ── Ready pose detection (when not swinging) ──
    if (!state.active) {
        bool foundReady = false;
        for (const Npc& npc : npcs.all()) {
            if (npc.body.dead) continue;
